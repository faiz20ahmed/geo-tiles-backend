...
if (!HOST_URL)
  console.warn("⚠️ HOST_URL not set in env, Webhook may fail");
...
bot.onText(/\/register (.+)/, async (msg, match) => {
  ...
  try {
    ...
    bot.sendMessage(
      chatId,
      "✅ تم تسجيلك بنجاح! استخدم /login <password>"
    );
...
app.get('/download', async (req, res) => {
  ...
  try {
    ...
    if (user.balance_basic < 1)
      throw new Error("INSUFFICIENT_BALANCE");

    // 🔥 هنا التعديل المهم
    const filePath = `files/${film_code}.tif`;
